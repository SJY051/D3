...
| M-04 | 7 | Run and Submit use an isolated Judge0 boundary with six explicit language mappings and classified failures | D3-JDG-001 | Six runtime smokes and failure normalization tests | Judge v1 contract and fake normalization tests; dedicated Judge0 CE 1.13.1 host passed the pinned six-runtime and isolation smoke; real application adapter remains incomplete in #13 |
| M-05 | 7 | A committed result explains speed, dynamic efficiency and submission discipline without claiming measured Big-O | D3-BTL-003 | Formula examples, boundary tests and repeatability sample | Safe evidence contract and fake size tiers; host calibration `UNKNOWN` |
