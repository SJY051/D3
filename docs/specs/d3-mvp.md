...
Last verified: 2026-08-19 against `c738cd8`, issues #17/#59 and merged PRs #70/#75/#76/#79/#89/#91/#92/#96
...
| M-02 | 7 | Two users selecting the same language enter one ranked match | D3-BTL-001 | Two-client matchmaking test and queue trace | PARTIAL PASS: deterministic widening-window policy, Redis TTL coordination, authenticated HTTP contract, idempotent PostgreSQL match creation, concurrent single-active-match fencing and Identity `battle.play` issuance have active tests; the `25359ad` two-browser rehearsal recorded the live same-language queue-to-match trace; fresh `c738cd8` two-session acceptance passed on 2026-08-19 (match `30e8b599`) |
...
| M-04 | 7 | Run and Submit use an isolated Judge0 boundary with six explicit language mappings and classified failures | D3-JDG-001 | Six runtime smokes and failure normalization tests | PASS at the application adapter boundary: authenticated Judge HTTP v1, deterministic fake, `RUN`/`SUBMIT` case selection, six pinned mappings, classified failures, durable evidence/outbox and private Battle forwarding have automated coverage. Issue #59 SSM command `a38944c3-8073-47de-b414-f3bd610acdf8` ran the production `HttpJudge0Client` and `Judge0ExecutionAdapter` for all six runtimes over an exact source-security-group route to the designated Judge0 CE 1.13.1 host; output privacy passed, the temporary runner was removed, and the host returned to zero ingress. A deployed Judge service remains outside the current local-only application topology |
