...
Issue #15 now has a partial ranked-entry slice: the six-language matcher uses a configurable widening rating window; Redis owns only expiring queue, sequence and lease keys; PostgreSQL atomically creates one deterministic match for two user-scoped tickets; and the Battle HTTP boundary derives player identity from the JWT subject while requiring `battle.play`. PostgreSQL transaction-scoped advisory locks serialize assignment by player ID, then recheck active `LOBBY` through `JUDGING` participation so a new ticket cannot place one player into a second active ranked match. A stale conflicting queue entry is removed without discarding its unmatched opponent. Match and player rows are read through one joined PostgreSQL statement so an aggregate version cannot be combined with participants from a later commit. The aggregate snapshot and optimistic version are the persistence and WebSocket ordering source. The original open-payload Battle event v1 remains unchanged; a participant-scoped application projector emits the closed `MATCH_SNAPSHOT` v2 shape per aggregate version with server time, self/opponent readiness and connection state. V2 exposes the authenticated self ID but omits the opponent ID, connection fencing generations, literals, incident references and source. Participant-bound command receipts make READY/reconnect/surrender retries idempotent, and the second READY commits ordered READY then RUNNING versions in one transaction. Identity token issuance for `battle.play`, Gateway/browser integration, WebSocket authorization/fan-out and two-session evidence remain **PENDING**, so this is not an end-to-end ranked-match claim.
