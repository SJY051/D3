...
Each domain database now has a service-owned Flyway V1 migration aligned with this document's logical ERD. Identity keeps refresh-token rotation inside one user's single-use lineage. Battle derives accepted-submission state from its owned judge-job correlation instead of maintaining a second pointer. Community stores the two match seats as distinct typed UUIDs and reconstructs the public event array at its boundary. Container tests execute each migration and its ownership invariants; Battle additionally proves Redis and Kafka connectivity. These migrations provide persistence boundaries, not completed Identity, Battle or Community behavior.
