...
Each domain database now has a service-owned forward-only Flyway chain aligned with this document's logical ERD. Previously applied V1 migrations remain immutable; Identity, Battle and Community reach the normalized model through V2, while Judge preserves V1/V2 and strengthens its model through V3. Identity keeps refresh-token rotation inside one user's single-use lineage. Battle derives accepted-submission state from its owned judge-job correlation instead of maintaining a second pointer. Community stores the two match seats as distinct typed UUIDs and reconstructs the public event array at its boundary. Container tests prove both fresh installation and upgrade with existing data; Battle additionally proves Redis and Kafka connectivity. These migrations provide persistence boundaries, not completed Identity, Battle or Community behavior.
