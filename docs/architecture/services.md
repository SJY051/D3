...
Issue #15 now has a partial ranked-entry slice: the six-language matcher uses a configurable widening rating window; Redis owns only expiring queue, sequence and lease keys; PostgreSQL atomically creates one deterministic match for two user-scoped tickets; and the Battle HTTP boundary derives player identity from the JWT subject while requiring `battle.play`. The aggregate snapshot and optimistic version are the persistence and future WebSocket ordering source. Participant-bound command receipts make READY/reconnect/surrender retries idempotent, and the second READY commits ordered READY then RUNNING versions in one transaction. Identity token issuance for `battle.play`, Gateway/browser integration, WebSocket authorization/fan-out and two-session evidence remain **PENDING**, so this is not an end-to-end ranked-match claim.
