...
Each domain database has a service-owned forward-only Flyway chain aligned with this document's logical ERD. Previously applied V1 migrations remain immutable; Identity and Community reach the normalized model through V2, Judge reaches V3, and Battle reaches V5. Upgrade normalization preserves ambiguous refresh lineages, accepted-submission pointers, duplicate judge correlations, legacy null attempt numbers and duplicate runtime evidence in explicit legacy records. Untypable Community projections remain as `REBUILD_REQUIRED` placeholders referenced by existing posts while their original payloads enter a rebuild queue for authoritative event replay. Battle V3 adds user-scoped queue tickets plus restorable lobby, readiness and reconnect fields, and backfills existing started matches before enforcing the new invariants. Battle V4 adds participant-bound command receipts without changing V3. Battle V5 assigns explicit reasons to valid legacy terminal results, including imported draws, before enforcing terminal-result consistency. Container tests prove fresh installation and adversarial existing-data upgrade; Battle additionally proves Redis and Kafka connectivity. These migrations provide persistence boundaries, not completed Identity, Battle or Community behavior.

Issue #15 now has a partial ranked-entry slice: the six-language matcher uses a configurable widening rating window; Redis owns only expiring queue, sequence and lease keys; PostgreSQL atomically creates one deterministic match for two user-scoped tickets; and the Battle HTTP boundary derives player identity from the JWT subject while requiring `battle.play`. PostgreSQL transaction-scoped advisory locks serialize assignment by player ID, then recheck active `LOBBY` through `JUDGING` participation so a new ticket cannot place one player into a second active ranked match. A stale conflicting queue entry is removed without discarding its unmatched opponent. The aggregate snapshot and optimistic version are the persistence and WebSocket ordering source. A participant-scoped application projector emits one closed `MATCH_SNAPSHOT` v1 shape per aggregate version with server time, self/opponent readiness and connection state; it excludes connection fencing generations, incident references and source. Participant-bound command receipts make READY/reconnect/surrender retries idempotent, and the second READY commits ordered READY then RUNNING versions in one transaction. Identity token issuance for `battle.play`, Gateway/browser integration, WebSocket authorization/fan-out and two-session evidence remain **PENDING**, so this is not an end-to-end ranked-match claim.
