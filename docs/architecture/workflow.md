...
The ranked queue segment is partially active under issue #15. Authenticated `POST /api/v1/battle/ranked/queue` accepts only a language and user-scoped idempotency key; player identity comes from the JWT subject. Redis lease and entry keys expire, while a successful pair creates one PostgreSQL lobby and removes its queue entries only after commit. Replaying either user's ticket converges on that committed match. The first browser can therefore poll by replaying the same POST. Battle can already project the latest committed aggregate as a participant-specific full snapshot whose `sequence` equals `aggregateVersion`; reconnect therefore needs the latest snapshot, not reconstruction from partial client events. Identity does not yet issue the required `battle.play` scope, and realtime transport remains inactive until the Gateway-compatible browser WebSocket credential transport is approved under issue #27.
