...
| Battle realtime projection privacy | PARTIAL PASS in unit, configuration and Gateway runtime tests | Gateway accepts one compact JWT subprotocol only on the exact Battle match path, authenticates it, and removes it before proxying. Battle requires `battle.play`, one exact origin and a PostgreSQL-backed participant lookup before upgrade. Viewer-specific v2 snapshots exclude opponent and winner UUIDs, source, literals, connection generations and incident references; stale versions are suppressed and failed sessions are isolated. Identity issuance and a live two-browser handshake remain NOT RUN |
