...
| Adapter integration | PostgreSQL, Redis, Kafka, outbox/inbox | Forward-only service migrations run on PostgreSQL Testcontainers; Battle proves Redis TTL/lease coordination, two-user matching, idempotent match creation, upgrade backfill and optimistic lifecycle persistence; Judge adds transactional outbox and producer behavior | Real-container transaction, uniqueness and retry evidence | PARTIAL PASS: ranked entry and Judge producer active; Battle terminal outbox and realtime adapters pending |
| Contract | HTTP, events, WebSocket | Ten parseable versioned documents; Judge HTTP behavior and Battle ranked-queue auth/identity/error behavior have active tests | Producer/consumer compatibility and negative samples | PARTIAL PASS: Judge HTTP and Battle ranked queue active; WebSocket and Battle event consumers incomplete |
