...
| Slice | Current result | What it proves |
|---|---|---|
| Versioned Kafka consumer | PASS | A real Kafka delivery of `match.finished.v1` reaches Community and replay converges to one inbox row and one ACTIVE projection |
...
| Contract privacy | PASS | Strict parsing rejects unknown private fields, duplicate JSON fields and aggregate/match correlation mismatches before persistence |
