...
|---|---|---|---|---|
| Domain unit | Match state, outcome, rating, energy | Active Battle lifecycle, versioned score/rating calculators, plus deterministic fake and real-Judge normalization slices; remaining requirement skeletons disabled | Deterministic examples, boundaries and clock control | PARTIAL PASS: Battle outcome and rank examples active; attack and persistence requirements remain skipped |
| Adapter integration | PostgreSQL, Redis, Kafka, outbox/inbox | Forward-only service migrations run on PostgreSQL Testcontainers; Battle proves Redis TTL/lease coordination, two-user matching, per-player concurrent active-match fencing, idempotent match creation, legacy clock/result restoration, single-snapshot aggregate reads, optimistic lifecycle persistence, transactional READY/SURRENDER replay, no-op receipt-growth rejection, PostgreSQL transport-generation allocation, stale-session command rejection, retained disconnect-conflict retry, pre-start close convergence, exact-boundary autonomous deadline advancement, stable match/reconnect priority across a later reconnect, cross-instance `SKIP LOCKED` claims, whole-batch commit before asynchronous deadline fan-out, generation-fenced Redis retry, periodic PostgreSQL repair after a missed subscriber notification, snapshot-preparation deferral without disconnect, unchecked transport-failure eviction, and one snapshot notification reaching two independent Battle listener containers; Judge adds transactional outbox and producer behavior | Real-container transaction, uniqueness and retry evidence | PARTIAL PASS: ranked entry, initial client commands, transport-generation fencing, autonomous deadline advancement and retrying cross-instance outbound notification plus Judge producer active; Battle terminal outbox pending |
| Contract | HTTP, events, WebSocket | Twelve parseable versioned documents; the original Battle event v1 is preserved, the closed participant snapshot is v2 and the closed command v2 permits only READY/SURRENDER; negative samples reject client-owned identity/source, undeclared command types and wrong versions. Judge HTTP behavior, Battle ranked-queue auth/identity/error behavior, Gateway canonical Identity ingress and browser WebSocket credential conversion have active tests | Producer/consumer compatibility and negative samples | PARTIAL PASS: Gateway/Battle WebSocket auth, membership, projection, initial client-command and server-owned connection lifecycle tests are active; Identity issuance and Battle event consumers remain incomplete |
