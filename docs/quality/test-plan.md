...
Last verified: 2026-08-19 against `ab71e2b`, the `25359ad` two-browser rehearsal, issue #59 route smoke, and PR #98 early-judging tests
...
| Browser | Ranked golden path and privacy | Playwright covers the approved P0 routes and Battle attack source integrity. The isolated `25359ad` rehearsal drove two browser sessions through sign-in, feed, ranked queue, Run/Submit, reconnect, result, rating/RP, result post and public record with the deterministic fake judge | Two independent sessions with fake judge; capture the both-accepted early-JUDGING transition | PASS for the recorded `25359ad` rehearsal; fresh `ab71e2b` acceptance, submission-verdict/early-JUDGING capture and live attack/Surrender traces remain PENDING/NOT RUN |
...
| Demo preflight | Full-stack dependency readiness | `pnpm local:start` plus HTTP/TCP checker | All required live dependencies pass on frozen build | PASS for the isolated `25359ad` rehearsal; rerun on `ab71e2b` remains PENDING |
...
| Browser demonstration | Fresh two-session fake-Judge run on `ab71e2b` | PENDING: record self-only verdict/lock, both accepted, early `JUDGING`, result, and record |
