...
- Last verified: 2026-08-14 against `origin/main` at `069e2ce`, live GitHub state and AWS schedule group `d3-judge0-20260814-16`
...
| [#36](https://github.com/SJY051/D3/pull/36) | `SJY051` | **MERGED — PARTIAL #27** | Gateway canonical Identity ingress와 Judge service-principal 식별을 회귀 테스트로 고정했다. 모듈 전체 테스트, scaffold, targeted security diff scan 0건과 CI 5/5 후 2026-08-14에 squash merge했다. Merge commit: `eeb33c987b79bfbfef207c04fb8af6747bbb7759`. Identity cookie/key lifecycle과 실제 user/service token 발급은 #26 통합 뒤에 남는다. |
...
| B1 Ranked realtime lifecycle | [#15](https://github.com/SJY051/D3/issues/15), PR #33/#38/#40 / M-02, M-03 | **윤서진** | 임수혁 backend 지원 | #33/#36/#38/#40 기반 병합 완료 | 두 client 매칭, server clock, reconnect, surrender, incident void, authenticated WS, Judge correlation | **진행 중**: authenticated outbound WebSocket auth/membership, Redis cross-instance fan-out, bounded delivery, replay와 closed READY/SURRENDER command deterministic evidence PASS; transport-owned PostgreSQL connection generation, stale-close/command fencing과 pre-start close convergence는 구현·검증 중; per-viewer session bound는 #37, autonomous reconnect expiry, Identity-issued token, Judge correlation과 live 두 세션 증거는 PENDING |
...
| 2 | #15 authenticated realtime seam과 Judge correlation | 3 | 3 | 2 | 2 | 3 | **18** | outbound authorization/fan-out/replay는 #38, closed READY/SURRENDER command는 #40에서 고정했다. 현재 조각은 transport-owned disconnect fencing이며, Judge correlation은 #26 발급 계약 뒤에 연결한다. |
...
1. **임수혁 (`GledoubleN`):** PR #26을 `069e2ce` 이후 최신 `main`에 rebase하고 V1을 다시 쓰지 않은 채 migration/security 충돌을 해결한다. 재검증 전 기존 CI 성공을 현재 통과 증거로 재사용하지 않는다.
2. **윤서진:** 매일 23:55 정지와 다음 운영일 09:00 시작 결과를 확인하고, Judge0 6-runtime smoke를 별도 증거로 남긴다.
3. **윤서진:** #40까지 병합된 #15 기반 위에서 transport-owned disconnect-generation 경계를 고정하고, 다음 조각에서 autonomous reconnect expiry를 연결한다.
