...
The suite proves unauthenticated rejection, authenticated version/worker discovery, network opt-in rejection, server-side maximum rejection, a blocked outbound socket from executed code after the host reaches the same endpoint, six hello-world executions, the same deterministic sum problem in all six languages, and wrong-answer, compilation, runtime, CPU-time, wall-time, memory, process/thread, stack, and file-size boundaries. Omission-based cases separately exercise the configured defaults; memory, process/thread, stack and file-size defaults use a below-bound success and above-bound failure pair. The one-second CPU case must report between 0.9 and 1.5 CPU seconds, distinguishing it from the two-second default, while the omitted CPU case must report between 1.8 and 2.6 seconds. The one-second wall case sleeps without consuming that CPU budget and must still time out while reporting at most 0.2 CPU seconds; its omitted counterpart sleeps past the five-second default with the same low CPU evidence. Judge0 reports default memory pressure at exactly 262,144 KiB and the explicit lower-limit case at exactly 65,536 KiB; the Judge adapter owns normalization to D³ `MEMORY_LIMIT`. A live infrastructure outage is not induced by this suite; `PLATFORM_FAILURE` normalization is covered by the deterministic adapter test from PR #20.
