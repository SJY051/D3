...
@SpringBootTest(properties = {
        ...
        "d3.community.rating-changed-topic=rating.changed.v1.community-test",
        "d3.community.rating-changed-group=community-rating-changed-integration-test"
})
@Testcontainers
class CommunityRatingChangedKafkaIntegrationTest {
    ...
    private static final String TOPIC = "rating.changed.v1.community-test";
    ...
    @Test
    void d3Stat001ConsumesKafkaReplayIntoOneRatingProjection() throws Exception {
        JdbcClient jdbc = JdbcClient.create(dataSource);
        jdbc.sql("""
                        insert into profile_projection (user_id, handle, identity_source_version, projected_at)
                        values (cast(:userId as uuid), 'alice', 3, now())
                        """).param("userId", USER).update();
        ...
        await().atMost(Duration.ofSeconds(15)).untilAsserted(() -> {
            ...
            assertEquals("1450|60|GOLD|7", jdbc.sql("""
